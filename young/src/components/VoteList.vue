...
<script>
  ...
  export default {
    ...
    created () {
      ...
      window.addEventListener('scroller', this.handleScroll);
      this.getList()
    },
    methods: {
      handleScroll () {
        this.scroller = window.scrollY > 0;
        console.log('e',this.scroller)
      },
      getList (keywords='',page='') {
        ...
        axios.get('http://www.wecozhiku.com/gsh5/index.php/Home/JshApi/index' + '?keys=' + params.keys + '&sessID=' + this.$store.state.sessID + '&p=' + params.p + '&l=' + params.l).then(res => {
          if (res.data.status === 0) {
            let rlist = res.data.list
            console.log('rlist',rlist)
            console.log('1',this.bList)
            rlist.forEach( item =>{
              this.bList.push(item)
            })
            console.log('2',this.bList)
